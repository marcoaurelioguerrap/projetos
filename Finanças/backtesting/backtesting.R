...
charts.RollingPerformance(R = retorno_indices_performance, width = 252)

chart.Drawdown(retorno_indices_performance, geometric = FALSE)

table.DownsideRisk(retorno_indices_performance)
table.AnnualizedReturns(retorno_indices_performance,geometric = FALSE)
SortinoRatio(retorno_indices_performance)
InformationRatio(retorno_indices_performance[,-1],retorno_indices_performance[,1])




SortinoRatio(retorno_indices_performance,)
SharpeRatio(retorno_indices_performance,annualize = TRUE)

risco_retorno <- data.frame( retorno = colMeans(retorno_indices_performance), des.pad. = colStdevs(retorno_indices_performance),
                             max_drawdown = t(maxDrawdown(retorno_indices_performance,geometric = FALSE)) , Sortino = t(SortinoRatio(retorno_indices_performance)))

SharpeRatio(retorno_indices_performance)


colnames(risco_retorno) <- c("retorno","des.pad.","max_drawdown","sortino")



ggplot(data = risco_retorno , aes(x=des.pad. ,y=((1+retorno)^252 -1) )) + 
  scale_y_continuous(name = 'Retorno Anualizado') +
  geom_point(aes(colour = sortino )) + 
  scale_colour_viridis_c(option = "magma") 

chart.RiskReturnScatter(retorno_indices_performance[,which(as.numeric(SortinoRatio(retorno_indices_performance) > .07)==1)],geometric = FALSE  )
